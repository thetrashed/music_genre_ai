...
pub fn build(b: *std.Build) void {
    ...
    // but does not run it.
    const wave_unit_tests = b.addTest(.{
        .root_source_file = .{ .path = "src/wav.zig" },
        .target = target,
        .optimize = optimize,
    });

    const run_wave_unit_tests = b.addRunArtifact(wave_unit_tests);

    const dft_unit_tests = b.addTest(.{
        .root_source_file = .{ .path = "src/dft.zig" },
        .target = target,
        .optimize = optimize,
    });
    ...
    const wave_test_step = b.step("wave_test", "Run unit tests for wave.zig");
    wave_test_step.dependOn(&run_wave_unit_tests.step);
